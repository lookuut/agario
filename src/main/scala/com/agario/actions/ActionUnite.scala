...
class ActionUnite(fragmentIds: Iterable[String], world : World) extends Action {
  ...
  def run () : Command = {
    ...
    if (fragments.size > 0) {
      val fragmentsCenter = fragments.map(_.get.circle.point).reduce(_ + _) * (1.0 / fragments.size)
...
object ActionUnite {
  def ttfFragments(fragments : Map[String, Fragment]) : Map[Int, (Point, Double)] = {
    fragments.
      values.
      map(f => (f.ttf.getOrElse(0), f)).
      groupBy(_._1).
      map{
        case (ttf, ttfFragments) =>
          val ttfAveragePoint = ttfFragments.map(_._2.circle.point).reduce(_ + _) * (1.0 / ttfFragments.size)
